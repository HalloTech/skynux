  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="container mx-auto flex flex-col lg:flex-row p-5 pt-20 max-sm:pt-12">
        {/* Left Sidebar - Pass session data */}
        <LeftSidebar 
          name={session?.user?.name || "User"} 
          email={session?.user?.email || ""}
          profilePic={session?.user?.image || "https://storage.googleapis.com/a1aa/image/J__Qk6moc86mHrxiCmew5lz8Q2nibBO5U9EesHT8lcw.jpg"}
        />
...
// Left Sidebar Component (now accepts props)
...
const LeftSidebar: React.FC<LeftSidebarProps> = ({ name, email, profilePic }) => {
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <div className="flex flex-col items-center">
          ...
          <div className="mt-4 text-center">
            ...
            <p className="text-gray-600">{name} | Software Engineer</p>
          </div>
        </div>
       ...
       <button 
          ...
          className="mt-4 w-1/3 px-4 py-2 bg-violet-500 text-white rounded-full hover:bg-violet-600"
        >
          Logout
        </button>
       </div>
...
// (Keep Post, RightSidebar, and FollowingItem components unchanged)
// Post Component
interface PostProps {
  username: string;
  name: string;
